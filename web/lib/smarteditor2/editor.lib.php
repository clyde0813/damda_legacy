...
function editor_html($id, $content, $is_dhtml_editor=true)
{
    ...
    global $_VAR_URL_WEB_LIB, $command, $BBS_VAL, $BBS_DATAS;
    static $js = true;

    if( 
        $is_dhtml_editor && $content && 
        (
        (!$command && (isset($BBS_VAL['dm_insert_content']) && !empty($BBS_VAL['dm_insert_content'])))
        || ($command == 'update' && isset($BBS_DATAS['wr_option']) && strpos($BBS_DATAS['wr_option'], 'html') === false )
        )
    ){       //글쓰기 기본 내용 처리
    ...
    $editor_url = $_VAR_URL_WEB_LIB.'smarteditor2';

    $html = "";

    if ($is_dhtml_editor && $js) {
        $html .= "\n".'<script src="'.$editor_url.'/js/service/HuskyEZCreator.js"></script>';
        $html .= "\n".'<script>oEditors = [], ed_nonce = "'.ft_nonce_create('smarteditor').'";</script>';
...
function chk_editor_js($id, $is_dhtml_editor=true)
{
    if ($is_dhtml_editor) {
        ...
        return "if (!{$id}_editor.value) { alert(\"내용을 입력해 주십시오1.\"); {$id}_editor.focus(); return false; }\n";
    }
}

if (!defined('FT_NONCE_UNIQUE_KEY'))
    define( 'FT_NONCE_UNIQUE_KEY' , sha1("dm") );
...
if(!function_exists('ft_nonce_create')){
    function ft_nonce_create( $action = '',$user='', $timeoutSeconds=FT_NONCE_DURATION ){
        ...
        setSession('token_'.FT_NONCE_SESSION_KEY, $secret);
...
if(!function_exists('ft_nonce_is_valid')){
    function ft_nonce_is_valid( $nonce, $action = '', $user='' ){
        ...
        $token = getSession('token_'.FT_NONCE_SESSION_KEY);
